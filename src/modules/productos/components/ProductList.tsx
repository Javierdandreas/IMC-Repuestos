...
import { BulkLocationAssigner } from "./BulkLocationAssigner";
...
export function ProductList({ products, totalPages = 1, currentPage = 1, totalCount = 0 }: Props) {
  ...
  const [openLocationAssigner, setOpenLocationAssigner] = useState(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(data.message || "No se pudo borrar el producto");
      ...
      <div className="space-y-6">
        ...
        <div className="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900/30">
            ...
            <BulkLocationAssigner
              ...
              onClose={() => setOpenLocationAssigner(false)}
