...
const PIEZA_FIELDS = [
  { id: 'codigo_pieza', label: 'Código de Pieza (Nro)', required: false },
  { id: 'descripcion', label: 'Descripción', required: true },
  { id: 'medida', label: 'Medida' },
  { id: 'imagen_url', label: 'URL de Imagen' },
  { id: 'subcategoria', label: 'Subcategoría' },
  { id: 'originales', label: 'Códigos Originales (sep. por coma)' },
  { id: 'equivalentes', label: 'Códigos Equivalentes (sep. por coma)' },
  { id: 'sustitutos', label: 'Códigos Sustitutos (sep. por coma)' },
];

export function ImportPiezaModal({ onClose }: { onClose: () => void }) {
      ...
      reader.onload = (e) => {
        try {
          ...
          headers.forEach(header => {
            ...
            if (h.includes('codigo') || h.includes('nro') || h === 'id') newMappings.codigo_pieza.csvHeader = header;
            if (h.includes('descripcion') || h === 'nombre') newMappings.descripcion.csvHeader = header;
            if (h.includes('medida')) newMappings.medida.csvHeader = header;
            if (h.includes('imagen') || h.includes('url') || h.includes('foto')) newMappings.imagen_url.csvHeader = header;
            if (h.includes('subcategoria')) newMappings.subcategoria.csvHeader = header;
            if (h.includes('original')) newMappings.originales.csvHeader = header;
            if (h.includes('equiv')) newMappings.equivalentes.csvHeader = header;
            if (h.includes('sustituto') || h.includes('sust')) newMappings.sustitutos.csvHeader = header;
          });
    ...
    try {
      ...
      const processImportData = async (allData: { data: any[] }) => {
        ...
        for (let i = 0; i < totalItems; i += BATCH_SIZE) {
          ...
          try {
            const res = await fetch("/api/piezas/import", {
              method: "POST",
              body: JSON.stringify({
                items: chunk,
                mappings,
            ...
            if (res.ok) {
              ...
              accumulatedResults.errors.push({
                ...
                codigo_pieza: `Lote ${Math.floor(i / BATCH_SIZE) + 1}`
              });
            }
          } catch (err: any) {
            accumulatedResults.errors.push({ row: i + 1, error: err.message, codigo_pieza: "ERROR" });
          }
      ...
      toast.error("Error crÃtico: " + error.message);
      setImporting(false);
    }
  };
      ...
      <div className="flex flex-col gap-6 p-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-2xl bg-emerald-500/10 p-5 border border-emerald-500/20">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500/60">Nuevas</span>
          </div>
          <div className="rounded-2xl bg-blue-500/10 p-5 border border-blue-500/20">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-blue-500/60">Actualizadas</span>
          </div>
            ...
            <div className="space-y-2">
              {results.errors.slice(0, 100).map((err, i) => (
                <div key={i} className="flex gap-2 text-xs text-red-400 p-2 bg-black/20 rounded-lg border border-red-500/10">
                  ...
                  <span className="font-mono text-[10px] ml-auto">{err.codigo_pieza}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <button onClick={onClose} className="h-14 w-full rounded-2xl bg-slate-900 font-black text-white transition hover:bg-slate-800 active:scale-95 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 shadow-xl">
          Cerrar Importador
        </button>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-5 animate-in fade-in duration-300">
        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-800">
          <div>
            <h3 className="text-lg font-black text-slate-900 dark:text-white">Mapeo de Piezas</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Vincula las columnas de tu archivo con los campos del sistema.</p>
          </div>
          ...
          {PIEZA_FIELDS.map((field) => (
            <div key={field.id} className="flex flex-col gap-2 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950/50">
              ...
              <select
                ...
                className="h-11 w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-4 text-xs font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition"
              >
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={() => setStep('upload')} className="h-12 flex-1 rounded-2xl bg-white dark:bg-slate-800 font-bold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-xs transition active:scale-95">
            AtrÃ¡s
          </button>
          <button
            onClick={handleImport}
            disabled={!mappings.descripcion.csvHeader}
            className="h-12 flex-[2] flex items-center justify-center gap-2 rounded-2xl bg-blue-600 font-black text-white shadow-lg shadow-blue-600/20 hover:bg-blue-500 disabled:opacity-50 text-xs uppercase tracking-widest transition-all active:scale-95"
          >
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-10 py-12 animate-in zoom-in duration-500">
        <div className="relative">
          <div className="h-40 w-40 animate-spin rounded-full border-4 border-slate-100 dark:border-slate-800 border-t-blue-500" />
        ...
        <div className="text-center">
          <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">Importando piezas</h3>
          <p className="text-sm text-slate-400 italic">&quot;Procesando {totalRows} registros...&quot;</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <label className="group relative flex h-64 cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50 transition hover:border-blue-400 hover:bg-blue-50/50 dark:border-slate-800 dark:bg-slate-950">
        ...
        <span className="text-base font-black text-slate-900 dark:text-white">Cargar CSV o Excel de Piezas</span>
        <span className="mt-1 text-xs font-medium text-slate-400">Sube tu catÃ¡logo tÃ©cnico para importar masivamente</span>
      </label>

      <div className="rounded-2xl bg-blue-50 p-4 border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800/40">
        <div className="flex gap-3">
          <HiExclamation className="h-5 w-5 text-blue-500 shrink-0" />
          <div className="text-xs text-blue-700 dark:text-blue-400 font-medium">
            <p className="font-black uppercase tracking-tight mb-1">InformaciÃ³n:</p>
            <p>Si el archivo contiene el <strong>Nro de Pieza</strong> y este ya existe, se actualizarÃ¡ la descripciÃ³n y medida.</p>
          </div>
        </div>
      </div>
    </div>
