...
export function NuevaOperacionWizard({ onClose, tipo }: NuevaOperacionWizardProps) {
  ...
  const handleScan = (e: React.FormEvent) => {
    ...
    // TODO: En VENTA, validar que la DB tenga esa serie DISPONIBLE.
    // Acá lo hacemos optimista, el back lo valida.
    // En COMPRA, estamos creando nuevas series con ese ID. (¿o son genéricas?)
    // PERO necesitamos saber A QUÉ PRODUCTO pertenece la serie.
      ...
      <div className="flex h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-slate-900">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${tipo === 'COMPRA' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}>
              <HiOutlineShoppingCart className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Nueva {tipo}</h2>
        ...
        <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
                ...
                <div className="space-y-5">
                    <div>
                        <label className="mb-1 flex items-center text-sm font-semibold text-slate-700 dark:text-slate-300">
                            <HiOutlineUser className="mr-1 h-4 w-4" /> Entidad ({tipo === 'COMPRA' ? 'Proveedor' : 'Cliente'})
                        </label>
                        <input type="text" value={entidadNombre} onChange={e => setEntidadNombre(e.target.value)} placeholder="Ej. Autopartes Cacho" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
                    </div>
                    <div>
                        <label className="mb-1 flex items-center text-sm font-semibold text-slate-700 dark:text-slate-300">
                            <HiOutlineDocumentText className="mr-1 h-4 w-4" /> N° Comprobante
                        </label>
                        <input type="text" value={comprobante} onChange={e => setComprobante(e.target.value)} placeholder="0001-00004561" className="w-full font-mono rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
                    </div>
                    <div>
                        <label className="mb-1 block text-sm font-semibold text-slate-700 dark:text-slate-300">Observaciones</label>
                        <textarea value={observacion} onChange={e => setObservacion(e.target.value)} rows={3} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-1 flex-col p-6 overflow-y-auto">
                 <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                     <h3 className="text-lg font-black text-slate-900 dark:text-white">Ítems de Operación</h3>
                             ...
                             <div key={item.producto.id} className="flex flex-col md:flex-row md:items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                                 ...
                                 <div className="flex items-center gap-4">
                                     <div>
                                         ...
                                         <input type="number" min="1" value={item.cantidad} onChange={e => updateCantidad(index, Number(e.target.value))} className="w-16 rounded-lg border border-slate-200 p-1.5 text-sm text-center font-bold dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
                                     </div>
                                 </div>

                                 {item.producto.usa_numero_serie && (
                                     <div className="flex-1">
                                          <label className="text-[10px] font-bold uppercase text-blue-500 block mb-1">Números de Serie (separados por coma)</label>
                                          <input 
                                             type="text" 
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900/50">
           ...
           <button onClick={handleSubmit} disabled={isSubmitting || carrito.length === 0} className="relative flex items-center justify-center gap-2 overflow-hidden rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition-all hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
             {isSubmitting ? (
                 <div className="h-5 w-5 animate-spin rounded-full border-2 border-white/20 border-t-white"></div>
             ) : (
                 <>
                  <HiCheck className="h-5 w-5" /> Procesar {tipo}
                 </>
             )}
           </button>
        </div>
