...
const navGroups: NavGroup[] = [
  {
    ...
    links: [
      { href: "/operaciones?tipo=COMPRA", label: "Compras", module: "operaciones" },
      {
  ...
  },
  {
    ...
    icon: HiOutlineCube,
    links: [
      { href: "/", label: "Items", module: "productos" },
      { href: "/piezas", label: "Piezas", module: "piezas" },
      { href: "/kits", label: "Kits", module: "kits" },
    ],
  },
  {
    label: "Gestión",
    ...
    links: [
      { href: "/importaciones", label: "Importaciones", module: "importaciones" },
      { href: "/marcas", label: "Marcas", module: "marcas" },
      { href: "/categorias", label: "Categorías", module: "categorias" },
      { href: "/ubicaciones", label: "Ubicaciones", module: "ubicaciones" },
    ],
  },
  {
    label: "Entidades",
    icon: HiOutlineUsers,
    links: [
      { href: "#", label: "Clientes", module: "clientes" },
...
export const Sidebar = () => {
    ...
    return navGroups
      .map((group) => ({
        ...
        links: group.links
          .map((link) => ({
            ...link,
            sublinks: link.sublinks?.filter((sub) => !sub.module || canViewModule(sub.module)),
          }))
          .filter((link) => {
            if (link.sublinks && link.sublinks.length > 0) {
              return Boolean(link.module ? canViewModule(link.module) : true) || link.sublinks.length > 0;
            }

            return !link.module || canViewModule(link.module);
          }),
      }))
  ...
  }, [visibleNavGroups, pathname, isLinkActive]);
  ...
  return (
    <>
      ...
      <aside
        ...
        className={`fixed top-0 left-0 z-50 h-screen border-r border-slate-300 bg-white transition-all duration-300 ease-in-out dark:border-slate-800 dark:bg-black/95 backdrop-blur-xl flex flex-col shadow-2xl md:shadow-none ${isMobileOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0'
            ...
            return (
              <div key={group.label} className="space-y-1">
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-bold transition-all ${isOpen && (!isCollapsed || isMobileOpen)
                              ...
                              {hasSublinks ? (
                                <>
                                  <button
                                    ...
                                    className={`flex w-full items-center justify-between rounded-lg py-2.5 px-3 text-xs font-bold transition-all ${isSubOpen ? "text-slate-900 dark:text-white" : "text-slate-500 hover:text-slate-900 dark:text-slate-400"
                                      }`}
                                  >
                                        ...
                                        {link.sublinks?.map(sub => (
                                          sub.external ? (
                                            ...
                                            <Link
                                              key={sub.href}
                                              href={sub.href}
                                              className={`block py-2 text-[11px] font-semibold transition-all ${isLinkActive(sub.href) ? "text-blue-600 dark:text-blue-400" : "text-slate-500 hover:text-slate-900"
                                                }`}
                                            >
                                              {sub.label}
                                            </Link>
                                ...
                                link.external ? (
                                  ...
                                  <Link
                                    href={link.href}
                                    className={`block py-2.5 px-3 text-xs font-semibold transition-all ${active
